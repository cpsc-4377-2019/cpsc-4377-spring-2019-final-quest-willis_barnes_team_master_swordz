...
bool PhysicsDevice::setTransform(BodyComponent* object, Position pos, EngineFloat angle)
//**************************************
{
...
bool PhysicsDevice::accelerate(const BodyComponent * object, Position forceVec)
{
...
//**************************************
//sets angular velocity
bool PhysicsDevice::setLinearVelocity(BodyComponent* object, Position linearVelociy)
//**************************************
{
	b2Body* body = FindBody(object);
	body->SetAwake(true);
...
EngineFloat PhysicsDevice::getAngularVelocity(BodyComponent* object) const
//**************************************
{
	b2Body* body = FindBody(object);
...
Position PhysicsDevice::getPosition(BodyComponent* object) const
//**************************************
{
	//b2Body* body = FindBody(object);
...
bool PhysicsDevice::createFixture(BodyComponent* object, BodyInitializer presets)
//**************************************
{
	std::shared_ptr<Texture> texture = object->getOwner()->getComponent<SpriteComponent>()->texture;
	...
	presets.pos.x -= (texture->getWidth() / 2);
	//subtract off half the height
	presets.pos.y -= (texture->getHeight() / 2);
	//**********************************************************************************

	// set starting position & angle
	bd->position.Set(RW2PW(presets.pos.x), RW2PW(presets.pos.y));
	...
	shapefd.density = 15.0f;//presets.physics.density;
...
Position PhysicsDevice::alignCenters(BodyComponent* object) const
//**************************************
{
	b2Body* body = FindBody(object);
...
Object* PhysicsDevice::RayCast(BodyComponent* object, Position target) {
	Object* seen = nullptr;
	...
	world->RayCast(&callback, pos, b2target);
	if (callback.fixture) {
		b2Body* seenBody = callback.fixture->GetBody();
		BodyComponent* seenObject = static_cast<BodyComponent*>(seenBody->GetUserData());
		seen = seenObject->getOwner();
	}

	return seen;
}

b2Vec2 PhysicsDevice::normalize(b2Vec2 vec) {
